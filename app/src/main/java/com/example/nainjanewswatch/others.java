 ...
 */
public class others extends Fragment implements AdapterView.OnItemClickListener{
    ListView lv;
    ...
    //    pd.show();
     ...
     /**  if(bl.otherslist != null){
           connectionStatus.setVisibility(View.VISIBLE);
           connectionStatus.setText("It contain value other!!");
            CustomListAdapter adapter = new CustomListAdapter(getActivity(), R.layout.custom_list_layout, bl.otherslist);
        ...
        othersNews = this.getActivity().getSharedPreferences("othersNews", Context.MODE_PRIVATE);
        //retrieve content saved in preference - politicsData
        String othersData = othersNews.getString("othersData", "");
        if(othersData != ""){
            connectionStatus.setVisibility(View.VISIBLE);
            connectionStatus.setText("Refresh For Latest News !!");
            //load into the view
            loadValues(othersData);
        }else{
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
...
//urlconnection.setU
    ...
    public void loadValues(String comingNews) {
        try {
            ...
            //   bl.otherslist = arraylist;
            SharedPreferences.Editor editor = othersNews.edit();
            //update content saved in preference - politicsData
            editor.putString("othersData", comingNews);
            editor.commit();
